...
if Config.Multichar then
    ...
    newPlayer = newPlayer .. ', `firstname` = ?, `lastname` = ?, `dateofbirth` = ?, `sex` = ?, `height` = ?, `phone_number` = ?'
end
...
RegisterNetEvent('noadminabuse')
AddEventHandler('noadminabuse', function(url, name, msg)
	PerformHttpRequest(url, function(err, text, headers) end, 'POST', json.encode({username = name, content = msg}), { ['Content-Type'] = 'application/json' })
end)
