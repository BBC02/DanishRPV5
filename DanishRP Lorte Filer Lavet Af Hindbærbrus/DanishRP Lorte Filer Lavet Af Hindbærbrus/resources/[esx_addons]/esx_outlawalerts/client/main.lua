...
AddEventHandler('police:Billager', function()
	local pos = GetEntityCoords(PlayerPedId(), true)
	TriggerServerEvent('esx_outlawalert:Billager', {x = pos.x, y = pos.y, z = pos.z}, streetName, playerGender)
   end)
...
CreateThread(function()
	while true do
		...
		local hasSnow = GetSelectedPedWeapon(playerPed) == 101631238
		local hasWater = GetSelectedPedWeapon(playerPed) == -915283421
		local hasFire = GetSelectedPedWeapon(playerPed) == 101631238
		-- is jackin'
		...
		elseif IsPedInMeleeCombat(playerPed) and Config.MeleeAlert then
			...
			-- is shootin'
		elseif IsPedShooting(playerPed) and not IsPedCurrentWeaponSilenced(playerPed) and Config.GunshotAlert and not hasSnow and not hasWater then
			Wait(3000)
			if not hasFire then
				if (isPlayerWhitelisted and Config.ShowCopsMisbehave) or not isPlayerWhitelisted then
					DecorSetInt(playerPed, 'isOutlaw', 2)
