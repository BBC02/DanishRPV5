...
ESX.RegisterServerCallback('SmallTattoos:PurchaseTattoo', function(source, cb, tattooList, price, tattoo, tattooName)
	...
	if xPlayer.getAccount('bank').money >= price then
		...
		TriggerClientEvent('esx:showNotification', source, "Du købte ~y~" .. tattooName .. "~s~ tattoo for ~g~DKK" .. price)
		cb(true)
	else
		TriggerClientEvent('esx:showNotification', source, "Du har ikke nok penge til og købe den her tatto")
		cb(false)
	end
end)
