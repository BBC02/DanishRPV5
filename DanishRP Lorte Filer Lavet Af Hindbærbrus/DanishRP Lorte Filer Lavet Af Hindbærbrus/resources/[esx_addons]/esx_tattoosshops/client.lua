...
Citizen.CreateThread(function()
	AddTextEntry("ParaTattoos", "Tattoverings Butik")
...
function GetNaked() 
	for k, v in pairs(Config.outfits) do
		ESX.TriggerServerCallback(
			"fivem-appearance:getPlayerSkin",
			function(skin)
				if skin.model == v.model then
					exports['fivem-appearance']:setPedComponents(PlayerPedId(), {v.torso,v.undershirt,v.pants,v.shoes,v.bag,v.accesories,v.kevlar,v.badge,v.arms})
				end
			end
		)
	end
...
function RemoveTattoo(name, label)
	...
	ESX.ShowNotification("Du har fjernet din ~y~" .. GetLabelText(label) .. "~s~ tattoo")
end

function CreateScale(sType)
	if scaleString ~= sType and sType == "OpenShop" then
		scaleType = setupScaleform("instructional_buttons", "Åben tattoverings butik", 38)
		scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Ændre Kamera vinkel", 21, "Ændre gennemsigtighed", {90, 89}, "Køb/Fjern Tattoo", 191)
		scaleString = sType
	end
end
						...
						if GetEntityModel(PlayerPedId()) == `mp_m_freemode_01` then
							if tattoo.HashNameMale ~= '' then
								...
								if found then
									...
									local clicked, hovered = JayMenu.Button(GetLabelText(tattoo.Name), "~HUD_COLOUR_GREENDARK~DKK" .. price)
									if clicked then
