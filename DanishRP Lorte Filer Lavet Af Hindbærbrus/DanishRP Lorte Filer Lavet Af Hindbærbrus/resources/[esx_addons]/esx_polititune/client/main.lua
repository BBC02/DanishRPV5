...
Citizen.CreateThread(function()
    ...
    Citizen.Wait(10000)
    ESX.TriggerServerCallback('esx_polititune:getVehiclesPrices', function(vehicles)
        Vehicles = vehicles
	end)
end)
...
RegisterNetEvent('esx_polititune:cancelInstallMod')
AddEventHandler('esx_polititune:cancelInstallMod', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	ESX.Game.SetVehicleProperties(vehicle, myCar)
end)

RegisterNetEvent('esx_polititune:repair')
AddEventHandler('esx_polititune:repair', function()
	local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
	SetVehicleFixed(vehicle)
	myCar = ESX.Game.GetVehicleProperties(vehicle)
	startCar = ESX.Game.GetVehicleProperties(vehicle)
	TriggerServerEvent('esx_polititune:setStarterCarServer', startCar)
	mainPrice = {}
	totalPrice = 0
	GetAction({value = 'main'})
end)

RegisterNetEvent('esx_polititune:GoOutOfLS')
AddEventHandler('esx_polititune:GoOutOfLS', function()
...
RegisterNetEvent('esx_polititune:setVehicleProps')
AddEventHandler('esx_polititune:setVehicleProps', function(props)
...
function OpenPayMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pay_menu', {
		...
		if data.current.value == 'no' then
			...
			myCar = {}
			startCar = {}
			TriggerServerEvent('esx_polititune:removeStarterCarServer')
		elseif data.current.value == 'yeas' then
			ESX.TriggerServerCallback('esx_polititune:hasMoneyToExit', function(paid)
				if paid then
					menu.close()
					lsMenuIsShowed = false
					local vehicle = GetVehiclePedIsIn(PlayerPedId(), false)
					FreezeEntityPosition(vehicle, false)
					TriggerServerEvent('esx_polititune:refreshOwnedVehicle', myCar)
					TriggerServerEvent('esx_polititune:removeStarterCarServer')
					myCar = {}
					startCar = {}
				end
		...
		for k,v in pairs(Config.Menus) do
			...
			if k == data.current.modType or isRimMod then
				if (data.current.label == " ".._U('by_default') and GetVehicleMod(vehicle, v.modType) ~= -1) or string.match(data.current.label, _U('installed')) then
					...
					if ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'redline' then
...
RegisterNetEvent('gris:ls')
AddEventHandler('gris:ls', function()
		GetAction({
			value = 'cosmetics'
		})
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local lastZone    = nil
			local vehicle = GetVehiclePedIsIn(playerPed, false)
			if (ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'redline') or Config.IsMechanicJobOnly == false then
				for k,v in pairs(Config.Zones) do
			...
			if IsControlJustReleased(0, Keys['E']) and not lsMenuIsShowed and isInLSMarker then
				if not IsSpecial or (ESX.PlayerData.job ~= nil and ESX.PlayerData.job.name == 'redline') then
