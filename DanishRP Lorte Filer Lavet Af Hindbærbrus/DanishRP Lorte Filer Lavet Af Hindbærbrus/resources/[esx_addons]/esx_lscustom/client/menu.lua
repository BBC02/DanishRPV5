						...
						if menu.selected == i then
							...
							if IsControlJustReleased(1,self.config.controls.menu_select) or IsDisabledControlJustReleased(1,self.config.controls.menu_select) then
								...
								if btn.menu ~= nil then
									...
									if btn.menu.parent == "Wheel type" then
										local buttons = {}
										local veh = GetVehiclePedIsIn(PlayerPedId(), false)
										local wheelType = GetWheelType(btn.name)

										SetVehicleWheelType(veh, wheelType)

										local modType = 23
										if IsThisModelABike(GetEntityModel(veh)) and btn.name == "Back wheel" then
											modType = 24
										end

										local wheels = GetNumVehicleMods(veh, modType)
										for i=0, wheels, 1 do
											local btn = {}
											
											local mod = i - 1
											local lbl = GetModTextLabel(veh, modType, mod)
