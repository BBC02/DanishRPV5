...
CancelDeathAnimationLoop = false
...
function Death()
	CreateThread(function()
        local playerPed = PlayerPedId()
        local deathLocation = GetEntityCoords(playerPed)
        ...
        if IsEntityDead(playerPed) then
            ...
            NetworkResurrectLocalPlayer(deathLocation.x, deathLocation.y, deathLocation.z, deathHeading, true, false)
            SetEntityInvincible(playerPed, true)
            
            TriggerEvent('esx_basicneeds:resetStatus')
            
            SetEntityCoords(playerPed, deathLocation)
            
            CancelDeathAnimationLoop = false
            PlayAnimation(playerPed, 'misslamar1dead_body', 'dead_idle', 4.0, 1.0, -1, 33, 0, 0, 0, 0)
            ...
            while LocalPlayer.state.isDead do
                ...
                if not CancelDeathAnimationLoop then
                    if not IsEntityPlayingAnim(playerPed, 'misslamar1dead_body', 'dead_idle', 3) then
                        deathLocation = GetEntityCoords(playerPed)
...
function PlayAnimation(ped, dict, anim, v1, v2, v3, v4, v5, v6, v7, v8)
...
function PlayAnimationLoop(ped, dict, anim, v1, v2, v3, v4, v5, v6, v7, v8)
...
exports('SetCancelDeathAnimationLoopValue', function(value)
    CancelDeathAnimationLoop = value
end)
