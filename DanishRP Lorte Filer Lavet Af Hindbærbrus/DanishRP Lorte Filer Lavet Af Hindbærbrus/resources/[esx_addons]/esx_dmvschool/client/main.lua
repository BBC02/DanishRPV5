...
function OpenDMVSchoolMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'dmvschool_actions', {
		...
		if data.current.value == 'theory_test' then
			local price = Config.Prices['dmv']
				ESX.TriggerServerCallback("dmvschool_niggapay", function(IsValid)
					if IsValid then
						StartTheoryTest()
					else
						exports['mythic_notify']:DoHudText('error', 'Du har ikke nok penge.')
					end
				end, price)
			menu.close()
		elseif data.current.value == 'drive_test' then
			local price = Config.Prices['drive']
			ESX.TriggerServerCallback("dmvschool_niggapay", function(IsValid)
				if IsValid then
