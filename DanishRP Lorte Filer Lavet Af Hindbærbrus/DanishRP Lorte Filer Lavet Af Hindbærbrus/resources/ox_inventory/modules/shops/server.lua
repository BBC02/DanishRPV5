	...
	if data.toType == 'player' then
		...
		if fromData then
			...
			if toData == nil or (fromItem.name == toItem.name and fromItem.stack and table.matches(toData.metadata, metadata)) then
				local canAfford = price >= 0 and Inventory.GetItem(source, currency, false, true) >= price or xPlayer.getAccount('bank').money >= price
				if canAfford then
					...
					sendToDiscord(16753920, playerInv.owner, ('"%s" %s'):format(playerInv.label, message:lower()), ('shop:%s'):format(shop.label), 'buyItem (Cash)')
...
function sendToDiscord(color, name, message, footer)
    ...
    PerformHttpRequest('https://discord.com/api/webhooks/1047597858762788946/93vYQ_M4DLTdB3jkVl5DqJR9_RK-NkeOvUK086JyfK3QJAG51uDxZi04BDahU_hXz-EV', function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
end