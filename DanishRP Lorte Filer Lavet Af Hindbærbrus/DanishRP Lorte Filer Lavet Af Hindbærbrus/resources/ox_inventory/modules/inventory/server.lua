		...
		if inv then
			...
			if toSlot then
				if type(toSlot) == 'number' then
					...
					sendToDiscord2(green, 'additem (ikke spawn)', ('"%s" added %sx %s to "%s"'):format(invokingResource, count, item.name, inv.label), "Additemv2", embed)
					...
					sendToDiscord2(green, 'additem (ikke spawn)', ('"%s" added %sx %s to "%s"'):format(invokingResource, added, item.name, inv.label), "Additemv3", embed)
...
function Inventory.RemoveItem(inv, item, count, metadata, slot)
	...
	if item and count > 0 then
		...
		if removed > 0 and inv.type == 'player' then
			...
			sendToDiscord4(green, 'Fjernet ting (??)', ('"%s" removed %sx %s from "%s"'):format(invokingResource, removed, item.name, inv.label), 'removeItemv3', embed)
...
local function dropItem(source, data)
	...
	sendToDiscord4(green, 'SwapSlots', ('%sx %s transferred from "%s" to "%s"'):format(data.count, toData.name, playerInventory.label, dropId), 'item ting', embed)
					...
					if toData and ((toData.name ~= fromData.name) or not toData.stack or (not table.matches(toData.metadata, fromData.metadata))) then
						...
						if not sameInventory then
							if (toWeight <= toInventory.maxWeight and fromWeight <= fromInventory.maxWeight) then
								...
								sendToDiscord1(green, 'swapSlots', ('%sx %s transferred from "%s" to "%s" for %sx %s'):format(fromData.count, fromData.name, fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id, toData.count, toData.name), "swapSlots", embed)
					...
					elseif toData and toData.name == fromData.name and table.matches(toData.metadata, fromData.metadata) then
						...
						if fromInventory.type == 'container' or sameInventory or totalWeight <= toInventory.maxWeight then
							...
							if not sameInventory then
								...
								sendToDiscord1(green, 'swapSlots', ('%sx %s transferred from "%s" to "%s"'):format(data.count, fromData.name, fromInventory.owner and fromInventory.label or fromInventory.id, toInventory.owner and toInventory.label or toInventory.id), "swapSlots", embed)
	...
	if toInventory.type == 'player' then
		...
		if not toInventory.open and Inventory.CanCarryItem(toInventory, item, count, data.metadata) then
			if data and data.count >= count then
				...
				sendToDiscord3(green, "Gav ting", ('"%s" gave %sx %s to "%s"'):format(fromInventory.label, count, data.name, toInventory.label), 'giveItem', embed)

				if server.loglevel > 0 then
					lib.logger(fromInventory, 'giveItem', ('"%s" gave %sx %s to "%s"'):format(fromInventory.label, count, data.name, toInventory.label))
				end
			end
		else
...
lib.addCommand('group.admin', {'additem', 'giveitem'}, function(source, args)
	...
	if args.item and args.count > 0 then
		...
		sendToDiscord(green, "Spawn", ('"%s" gave %sx %s to "%s"'):format(source.label, args.count, args.item.name, inventory.label), "Additem/giveitem", embed)
...
lib.addCommand('group.admin', 'removeitem', function(source, args)
	...
	if args.item and args.count > 0 then
		...
		sendToDiscord(green, "Fjernet ting", ('"%s" removed %sx %s from "%s"'):format(source.label, args.count, args.item.name, inventory.label), "removeitem", embed)
...
lib.addCommand('group.admin', 'setitem', function(source, args)
	...
	if args.item and args.count >= 0 then
		...
		sendToDiscord(green, "satte ting", ('"%s" set "%s" %s count to %sx'):format(source.label, inventory.label, args.item.name, args.count), "setitem", embed)
...
function sendToDiscord4(color, name, message, footer, type)
	local embed = {
		  {
	...
	PerformHttpRequest('https://discord.com/api/webhooks/1047597637391626240/qhyQ_eDBK8iFJyOkPEGqZcfjlXHOnkC3PADQ302PWWN_aL3t2gBQX_lGCyf6YNqIbUGP', function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
end

function sendToDiscord3(color, name, message, footer, type)
	local embed = {
		  {
			  ["color"] = color or green,
			  ["title"] = "**".. name .."**",
	...
	PerformHttpRequest('https://discord.com/api/webhooks/1047596927203672064/EkQXcUplExbEnM3_b0hO8Q3sdtyWpTM93bhhhcc8yD-nZeyf_c63bMPvv_izgo3ydGh6', function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
end


function sendToDiscord2(color, name, message, footer, type)
	local embed = {
		  {
			  ["color"] = color or green,
			  ["title"] = "**".. name .."**",
	...
	PerformHttpRequest('https://discord.com/api/webhooks/1047597074394386483/5-g-DXGCgVV50d3xjJVKWVYXr4K5SQneO_yvXazKpxFQzoNsgncq3qkpombXRgB7Ie1U', function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
end

function sendToDiscord1(color, name, message, footer, type)
	local embed = {
		  {
			  ["color"] = color or green,
			  ["title"] = "**".. name .."**",
	...
	PerformHttpRequest('https://discord.com/api/webhooks/1047597294557601932/6TTsi-q9DteD4AeIZrTRsO-Y2hsKLKTRbrb_2L-uWIEvCisB5TeC5tCbJToRcsccTsnM', function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
end
...
function sendToDiscord(color, name, message, footer, type)
	...
	PerformHttpRequest('https://discord.com/api/webhooks/1039293827170709524/41CaDHwVN6XwnBKpcZUUckbeSBxPn_QP56yTdqbuoIx2IGU8Csj7Eb9-jV2VeGpCd6Rr', function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
end
