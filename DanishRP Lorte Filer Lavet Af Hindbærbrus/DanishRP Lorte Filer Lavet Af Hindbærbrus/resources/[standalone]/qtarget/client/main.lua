...
function EnableTarget()
    ...
    local currentVehicle = IsPedInAnyVehicle(ped, false)
    
    if success or not IsControlEnabled(0, 24) then return end
    if not targetActive and (currentWeapon == -1569615261 or currentWeapon == 883325847 or currentWeapon == -1716189206 or currentWeapon == -1834847097 or currentWeapon == -538741184) and not currentVehicle then
        targetActive = true
            ...
            if entityType > 0 then
                ...
                elseif entityType == 2 and #(plyCoords - coords) <= 1.1 then
                    ...
                    if closestBone and #(coords - closestPos) <= data.distance then
						...
						local entityData = Types[entityType]
                        if entityData then
                            for o, data in pairs(entityData) do
                                if M.CheckOptions(data, entity, #(plyCoords - coords)) then
                                    local slot = #send_options + 1
                                    send_options[slot] = data
                                    send_options[slot].entity = entity
                                end
...
RegisterKeyMapping("+playerTarget", "[qtarget] Enable targeting~", "keyboard", "CAPITAL")
