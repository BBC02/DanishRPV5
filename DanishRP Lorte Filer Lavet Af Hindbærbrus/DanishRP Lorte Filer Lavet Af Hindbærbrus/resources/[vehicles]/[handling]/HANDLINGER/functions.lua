    ...
    elseif value == 'blindfold_toggle' then
        ...
        if distance ~= -1 and distance <= 3.0 then
            if IsPedInAnyVehicle(GetPlayerPed(player)) then
                exports['mythic_notify']:DoHudText('error', 'Du kan ikke give personer i et k??ret??j blindfold p??.', 5000)
            else
            ...
            exports['mythic_notify']:DoHudText('error', 'Ingen personer i n??rheden', 5000)
        end
	...
	options = {
		...
		},
		{
			...
			action = function(entity)
                ...
                TriggerServerEvent("cb_esx_personmenu:throwInTrunk", carry.targetSrc, netID)
			end,
			canInteract = function(vehicle)
                local isLocked = Entity(vehicle).state.isLocked
                local isTrunkOccupied = Entity(vehicle).state.isTrunkOccupied
		...
		},
		{
			...
			action = function(entity)
                local netID = NetworkGetNetworkIdFromEntity(entity)
                TriggerServerEvent("cb_esx_personmenu:takeOutTrunk", netID)
			end,
			canInteract = function(vehicle)
                local isLocked = Entity(vehicle).state.isLocked
                local isTrunkOccupied = Entity(vehicle).state.isTrunkOccupied
        ...
        --[[
		{
			...
			action = function(entity)
				...
				exports["input"]:Open("S??t Offset", ("ESX"), function(data)
...
RegisterNetEvent('cb_esx_personmenu:syncTarget')
AddEventHandler('cb_esx_personmenu:syncTarget', function(targetSrc)
    CarriedPerson(targetSrc)
end)

RegisterNetEvent('cb_esx_personmenu:stopCarry')
AddEventHandler('cb_esx_personmenu:stopCarry', function()
    StopCarry()
end)

RegisterNetEvent('cb_esx_personmenu:takeOutTrunk')
AddEventHandler('cb_esx_personmenu:takeOutTrunk', function()
    LeaveTrunk()
end)

RegisterNetEvent('cb_esx_personmenu:throwInTrunk')
AddEventHandler('cb_esx_personmenu:throwInTrunk', function(netID)
    isCarrying = false
    isCarried = false
    inTrunk = true
...
CarryPerson = function(closestID)
    ...
    TriggerServerEvent("cb_esx_personmenu:syncTarget", closestID)
    ...
    Citizen.CreateThread(function()
        while isCarrying do
            ...
            if IsControlJustReleased(0, Keys["E"]) and hasHelpText then
                TriggerServerEvent("cb_esx_personmenu:stopCarry", carry.targetSrc)
                carry.targetSrc = 0
...
CarriedPerson = function(targetSrc)
    ...
    --exports.drp_ambulance:SetCancelDeathAnimationLoopValue(true)
...
StopCarry = function()
    local playerPed = PlayerPedId()
   -- exports.drp_ambulance:SetCancelDeathAnimationLoopValue(false)
...
HideInTrunk = function(vehicle)
    Citizen.CreateThread(function()
        ...
        --TriggerServerEvent("cb_esx_personmenu:SetTrunkValue", netID, true)
...
LeaveTrunk = function()
   ...
   -- exports.drp_ambulance:SetCancelDeathAnimationLoopValue(false)
    ...
    if DoesEntityExist(vehicle) then
        ...
        local netID = NetworkGetNetworkIdFromEntity(vehicle)
        TriggerServerEvent("cb_esx_personmenu:SetTrunkValue", netID, false)
    end
    ...
    if closestPlayer ~= -1 and closestDistance <= 3.0 then
        if value == 'ziptie_on' then
            if not IsPedInAnyVehicle(GetPlayerPed(GetPlayerServerId(closestPlayer)), false) then
                TriggerServerEvent('cb_esx_personmenu:ziptie', GetPlayerServerId(closestPlayer))
                --TriggerEvent('RunCheck', '')
            else
                exports['mythic_notify']:SendAlert('error', 'Du kan ikke give strips p?? en der sider i en bil', 5000)
            end
        elseif value == 'ziptie_off' then
            if not IsPedInAnyVehicle(GetPlayerPed(GetPlayerServerId(closestPlayer)), false) then
                TriggerServerEvent('cb_esx_personmenu:RemoveZiptie', GetPlayerServerId(closestPlayer))
                --TriggerEvent('RunCheck', '')
            else
                exports['mythic_notify']:SendAlert('error', 'Du kan ikke tage strips af en der sider i en bil', 5000)
            end
        elseif value == 'put_vehicle' then
            TriggerServerEvent('cb_esx_personmenu:putInVehicle', GetPlayerServerId(closestPlayer))
            --TriggerEvent('RunCheck', '')
        elseif value == 'takeout_vehicle' then
            TriggerServerEvent('cb_esx_personmenu:OutVehicle', GetPlayerServerId(closestPlayer))
            --TriggerEvent('RunCheck', '')
        elseif value == 'drag_person' then
            TriggerServerEvent('cb_esx_personmenu:drag', GetPlayerServerId(closestPlayer))
        ...
        elseif value == 'check_pockets' then
            if IsPedArmed(PlayerPedId(), 7) then
                if IsEntityPlayingAnim(spiller, "random@arrests@busted", "idle_a", 3) or IsEntityPlayingAnim(spiller, 'misslamar1dead_body', 'dead_idle', 3) then
                    ...
                    exports['mythic_notify']:SendAlert('error', 'Spilleren er ikke p?? kn??.', 5000)
                end
            else
                exports['mythic_notify']:SendAlert('error', 'Du skal have et v??ben i h??nden.', 5000)
            end
        end
    else
        exports['mythic_notify']:SendAlert('error', 'Ingen personer i n??rheden', 5000)
        --ESX.ShowNotification("Ingen personer i n??rheden")
    end
end

RegisterNetEvent('cb_esx_personmenu:checkPocket')
AddEventHandler('cb_esx_personmenu:checkPocket', function(player)
    ...
    if not LocalPlayer.state.isDead and IsPedOnFoot(ped) then
        ...
        if target ~= -1 and distance ~= -1 and distance <= 2.0 then
            ...
            if IsEntityDead(player) or IsEntityPlayingAnim(player, 'misslamar1dead_body', 'dead_idle', 3) then
                ...
                exports['mythic_notify']:SendAlert('inform', 'Du kan ikke r??ve personen.', 5000)
            end
        
        elseif distance < 20 and distance > 2.0 then
            exports['mythic_notify']:SendAlert('inform', 'Du er for langt v??k fra personen', 5000)
        else
            exports['mythic_notify']:SendAlert('error', 'Ingen personer i n??rheden', 5000)
        end
    end
