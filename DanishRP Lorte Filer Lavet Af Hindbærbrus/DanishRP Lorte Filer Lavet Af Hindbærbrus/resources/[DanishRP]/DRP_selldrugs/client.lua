	...
	while true do
		if selling then
			if secondsRemaining > 0 then
				...
				exports['progressBars']:startUI(1000, "Sælger stofferne - 5 Sekunder")
			end
			Citizen.Wait(1000)
		end
		Citizen.Wait(100)
							...
							if pedType ~= 28 and IsPedAPlayer(ped) == false then
								...
								if distance <= 3 and ped  ~= GetPlayerPed(-1) and ped ~= oldped and IsControlJustPressed(1, 38) then
									...
									if distanceFromCity < Config.DistanceFromCity then
										...
										exports['mythic_notify']:DoLongHudText('error', _U('too_far_away_from_city'))
									end
...
--     AddEventHandler('showNotification', function(msg, notif)
--         exports["id_notify"]:notify({message = msg, type = notif})
-- end)

-- RegisterNetEvent('showNotification:advanced')
-- AddEventHandler('showNotification:advanced', function(title, msg, notif)
--         exports["id_notify"]:notify({title = title, message = msg, type = notif})
-- end)
        ...
        if pedIsTryingToSellDrugs then
			...
			elseif PlayerData.job ~= nil and PlayerData.job.name == 'police' and showcopsmisbehave then
				ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
					local sex = nil
					if skin.sex == 0 then
						sex = "person" --male/change it to your language
					else
						sex = "person" --female/change it to your language
					end
