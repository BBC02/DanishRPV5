...
local houseBlipsEnabled = true
RegisterCommand("togglehouseblips", function()
    houseBlipsEnabled = not houseBlipsEnabled

    if not houseBlipsEnabled then
        exports['mythic_notify']:DoHudText('error', 'Du har slået House Blips fra.', 5000)
    end

    if houseBlipsEnabled then
        exports['mythic_notify']:DoHudText('success', 'Du har slået House Blips til.', 5000)
    end

    RefreshBlips()
end, false)

RegisterCommand("addhouseblip", function(source, args, rawCommand)
  local job = GetPlayerJobName()
  if job == 'police' or job == 'realestateagent' then
    house = tonumber(args[1])
    for k,v in pairs(Houses) do
      if v.Id == house then
        if DoesBlipExist(v.blip) then
          RemoveBlip(v.blip)
        end
        v.blip = CreateBlip(v.Entry, 40, 1, "Markeret Hus")
      elseif v.Id == nil then
        exports['mythic_notify']:DoHudText('error', 'Ingen eksisterne huse med valgte ID.', 5000)
      end
    end
  end
end, false)

RegisterCommand("removehouseblip", function(source, args, rawCommand)
  local job = GetPlayerJobName()
  if job == 'police' or job == 'realestateagent' then
    house = tonumber(args[1])
    for k,v in pairs(Houses) do
...
RefreshBlips = function()
  local identifier = GetPlayerIdentifier()
    for i=1, #houseBlips, 1 do
        local blip = houseBlips[i]
    ...
    if houseBlipsEnabled then
        for k,v in pairs(Houses) do
            if v.Owner == identifier then
                v.blip = CreateBlip(v.Entry, 40, 2, "Huse du ejer")
            end

            for _k,_v in pairs(v.HouseKeys) do
              if _v.identifier == identifier then
                v.blip = CreateBlip(v.Entry, 40, 32, "Huse du har nøgler til")
              end
            end
            table.insert(houseBlips, v.blip)
        end
    end
end

CreateBlip = function(pos, sprite, color, text, display)
  if display == nil then
      display = 3
  end
...
function RefreshBlips()
  ...
  if Config.UseBlips then
    for _,house in pairs(Houses) do
      ...
      elseif house.HouseKeys and (house.HouseKeys == identifier) then
        text = "Huse du har nøgler til"
        color,sprite = GetBlipData("haskeys",house.Entry)
      else
        text = "Dødsbo"
