...
Savanha.MiniGame.SafeCracking.Toggle = function(status, locks, trigger)
    if status and not isCracking then
        isCracking = true
        lockAmounts = locks
        Savanha.MiniGame.SafeCracking.Run(trigger)
    else
        isCracking = false

        Savanha.MiniGame.SafeCracking.Reset()
    end
end

Savanha.MiniGame.SafeCracking.Run = function(trigger)
    dialRotation = 0.0
    Savanha.MiniGame.SafeCracking.Init()
    Savanha.MiniGame.SafeCracking.HelpText()
    Savanha.MiniGame.SafeCracking.Controlles(trigger)
end

Savanha.MiniGame.SafeCracking.Reset = function()
    isCracking = false
...
Savanha.MiniGame.SafeCracking.Init = function()
...
Savanha.MiniGame.SafeCracking.HelpText = function()
    Citizen.CreateThread(function()
        while isCracking do
            ...
            Savanha.MiniGame.SafeCracking.DrawButtons({
                {
                    ["label"] = "Højre Hurtigt",
...
Savanha.MiniGame.SafeCracking.Controlles = function(trigger)
    Citizen.CreateThread(function()
        while isCracking do
            ...
            elseif IsControlJustPressed(0, 172) then
                Savanha.MiniGame.SafeCracking.MoveSafeDial(true)
            elseif IsControlJustPressed(0, 173) then
                Savanha.MiniGame.SafeCracking.MoveSafeDial(false)
...
Savanha.MiniGame.SafeCracking.MoveSafeDial = function(clockwise)
    if clockwise then
...
Savanha.MiniGame.SafeCracking.DrawButtons = function(buttonsToDraw)
    Citizen.CreateThread(function()
...
RegisterCommand('safecrack', function(source, args)
    Savanha.MiniGame.SafeCracking.Toggle(true, 2, 'Savanha:HELLOWORLD')
end, false)
