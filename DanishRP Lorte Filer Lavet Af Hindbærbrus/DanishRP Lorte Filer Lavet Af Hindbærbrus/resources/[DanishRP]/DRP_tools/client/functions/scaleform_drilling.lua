...
Savanha.MiniGame.Drilling.ToggleDrilling = function(state, trigger)
    ...
    if state and trigger ~= nil then
        Savanha.MiniGame.Drilling.DisableMovement()
        Savanha.MiniGame.Drilling.DrawDrilling(trigger)
        Savanha.MiniGame.Drilling.KeyControleDrilling(trigger)
    else
        drilling = false
    end
end

Savanha.MiniGame.Drilling.DrawDrilling = function(trigger)
    Citizen.CreateThread(function()
        ...
        while drilling do
            ...
            if drilling then
                ...
                Savanha.MiniGame.Drilling.SetDrillPos(scaleform, drillpos)
                Savanha.MiniGame.Drilling.SetDrillTemp(scaleform, drilltemp)
                Savanha.MiniGame.Drilling.SetDrillDepth(scaleform, holedepth)
                Savanha.MiniGame.Drilling.SetDrillSpeed(scaleform, drillspeed)
                DrawScaleformMovieFullscreen(scaleform, 255, 255, 255, 200, 0)
                if holedepth >= 0.9 then
                    Savanha.MiniGame.Drilling.ToggleDrilling(false)
...
Savanha.MiniGame.Drilling.KeyControleDrilling = function(trigger)
    Citizen.CreateThread(function()
        while drilling do
            Citizen.Wait(0)
...
Savanha.MiniGame.Drilling.DisableMovement = function()
    Citizen.CreateThread(function()
        while drilling do
            Citizen.Wait(0)
...
RegisterCommand('drillgame', function(source, args)
    Savanha.MiniGame.Drilling.ToggleDrilling(true, 'Savanha:HELLOWORLD')
end, false)
