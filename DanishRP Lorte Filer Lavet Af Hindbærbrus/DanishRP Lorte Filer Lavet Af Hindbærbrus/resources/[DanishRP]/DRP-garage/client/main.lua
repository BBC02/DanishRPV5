	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedCars', function(ownedCars)
		if #ownedCars == 0 then
			...
			for _,v in pairs(ownedCars) do
				...
				local vehDamage	   = math.floor(health)
				local labelvehicle
				local labelvehicle2 = ('| <span style="color:red;">%s</span> - <span style="color:darkgoldenrod;">%s</span> | '):format(plate, vehicleName)..' '
				local labelvehicle2 = ('| <span style="color:red;">%s</span> - <span style="color:darkgoldenrod;">%s</span> | '):format(plate, vehicleName)..' '
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_car', {
			...
			if data.current.value == nil then
				...
				if data.current.value.stored then
					menu.close()
					local coords = GetEntityCoords(playerPed)
					SpawnVehicle(data.current.value.vehicle, data.current.value.plate, coords)
...
RegisterNetEvent('gris:garage', function()
		local elements = {}
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('esx_advancedgarage:storeVehicle', function(valid)
			if valid then
				...
				-- ESX.ShowNotification(_U('cannot_store_vehicle'))
				TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('cannot_store_vehicle'), length = 5000})
			end
		end, vehicleProps)
	else
		-- ESX.ShowNotification(_U('no_vehicle_to_enter'))
		TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('no_vehicle_to_enter'), length = 5000})
	end
end
					...
					ESX.TriggerServerCallback('esx_advancedgarage:checkMoneyCars', function(hasEnoughMoney)
						if hasEnoughMoney then
							if data.current.value == nil then
								...
								SpawnVehicle5(data.current.value, data.current.value.plate)
...
function StoreVehicle(vehicle, vehicleProps)
	...
	TriggerEvent('mythic_notify:client:SendAlert', { type = 'inform', text = _U('vehicle_in_garage'), length = 5000})
...
function GeneratePlate()
	...
	while true do
		...
		generatedPlate = 'QT ' ..GetRandomNumber(5)
		if doBreak then
			break
		end
		break
...
-- Spawn Vehicles
function SpawnVehicle(vehicle, plate, coords)
	local coords = GetEntityCoords(GetPlayerPed(-1))
	local Heading = GetEntityHeading(GetPlayerPed(-1))
	if ESX.Game.IsSpawnPointClear(coords, 4) then
		ESX.Game.SpawnVehicle(vehicle.model, coords, Heading, function(callback_vehicle)
			ESX.Game.SetVehicleProperties(callback_vehicle, vehicle)
			SetVehRadioStation(callback_vehicle, "OFF")
			SetVehicleUndriveable(callback_vehicle, false)
			SetVehicleEngineOn(callback_vehicle, true, true)
			exports['t1ger_keys']:SetVehicleLocked(callback_vehicle, 0)
		...
		ESX.ShowNotification('Der er et køretøj i vejen')
	end
...
function SpawnVehicle5(vehicle, plate)
...
Citizen.CreateThread(function()
	while true do
			...
			for k,v in pairs(Config.CarGarages) do
				...
				if distance < Config.Main.DrawDistance then
					...
					if Config.Cars.Markers.Points.Type ~= -1 then
						...
						if distance < Config.Main.DrawDistance - 18.5 then
							-- Check if the player is in vehicle
							if IsPedInAnyVehicle(PlayerPedId(), false) then
								 Draw3DText(v.Marker.x, v.Marker.y, v.Marker.z, "~w~[~g~E~w~] for at ~g~parkere ~w~dit ~g~køretøj")
								 --TriggerEvent('cd_drawtextui:ShowUI', 'show', "Tryk [E] for at parkere")
							else
								 Draw3DText(v.Marker.x, v.Marker.y, v.Marker.z, "~w~[~g~E~w~] for at ~g~tilgå garagen")
								 --TriggerEvent('cd_drawtextui:ShowUI', 'show', "Tryk [E] for at tilgå garage")
							end
						end
					...
					if distance < Config.Cars.Markers.Delete.x then
		...
		if (isInMarker and not HasAlreadyEnteredMarker) or (isInMarker and LastZone ~= currentZone) then
			...
			if currentZone == 'car_garage_point' then
				if IsPedInAnyVehicle(PlayerPedId(), true) then
					--TriggerEvent('cd_drawtextui:ShowUI', 'show', "Tryk [E] for at parkere")
				else
					--TriggerEvent('cd_drawtextui:ShowUI', 'show', "Tryk [E] for at tilgå garagen")
				end
			end
					...
					if IsPedInAnyVehicle(playerPed, false) then
						if IsThisModelACar(model) or IsThisModelABicycle(model) or IsThisModelABike(model) or IsThisModelAQuadbike(model) then
							if (GetPedInVehicleSeat(playerVeh, -1) == playerPed) then
								...
								-- ESX.ShowNotification(_U('driver_seat'))
								TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('driver_seat'), length = 5000})
							end
						else
							-- ESX.ShowNotification(_U('not_correct_veh'))
							TriggerEvent('mythic_notify:client:SendAlert', { type = 'error', text = _U('not_correct_veh'), length = 5000})
						end
					else
						ListOwnedCarsMenu()
					end
