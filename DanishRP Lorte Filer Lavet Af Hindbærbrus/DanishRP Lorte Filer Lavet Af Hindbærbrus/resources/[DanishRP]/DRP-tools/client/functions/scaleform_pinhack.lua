...
ESX.MiniGame.PinHack.Draw = function(sf, pos)
    PushScaleformMovieFunction(sf, "SET_DRILL_POSITION")
    ScaleformMovieMethodAddParamFloat(pos)
    EndScaleformMovieMethod()
end

ESX.MiniGame.PinHack.PositionCheck = function(arg1)
...
ESX.MiniGame.PinHack.CalcOne = function(arg1)
...
ESX.MiniGame.PinHack.CalcTwo = function(arg1, arg2, arg3)
    local number number = ((1.0 - Cos(ESX.MiniGame.PinHack.CalcThree((arg3 * 3.141593)))) * 0.5)
    return ((arg1 * (1 - number)) + (arg2 * number))
end

ESX.MiniGame.PinHack.CalcThree = function(arg1)
    return (arg1 * 57.29578)
end

ESX.MiniGame.PinHack.KeyListen = function()
    Citizen.CreateThread(function()
        local current = 1
        Dat_2[current].val0 = 0
        while hackingActive do
            ESX.MiniGame.PinHack.DisableKeys()
            if IsControlJustReleased(2, 237) then
                if ESX.MiniGame.PinHack.CalcOne(current) then
...
ESX.MiniGame.PinHack.Calc = function(dif)
    Citizen.CreateThread(function()
        while hackingActive do
            for i = 1, #Dat_2, 1 do
                ...
                if Dat_2[i].val0 == 0 or Dat_2[i].val4 then
                    Dat_1[i].val1 = ESX.MiniGame.PinHack.CalcTwo(0.744, 0.4, Dat_2[i].val2)
                end
                if stophack then
                    return
                end
...
ESX.MiniGame.PinHack.Draw = function(trigger)
    Citizen.CreateThread(function()
...
ESX.MiniGame.PinHack.DisableKeys = function()
...
ESX.MiniGame.PinHack.TogglePinHack = function(status, dif, trigger)
    if status then
        hackingActive = true
        ...
        ESX.MiniGame.PinHack.KeyListen()
        ESX.MiniGame.PinHack.Calc(loc1)
        ESX.MiniGame.PinHack.Draw(trigger)
...
AddEventHandler("ESX:PinHack", function()
    ESX.MiniGame.PinHack.TogglePinHack(true, 6)
end)

RegisterCommand('pinhackgame', function(source, args)
    ESX.MiniGame.PinHack.TogglePinHack(true, 6, 'hello:world')
end, false)
