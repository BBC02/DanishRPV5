...
Voltlab.Start = function(time, callback)
    CreateThread(function()
        ...
        Voltlab.StartMinigame(time, callback)
    end)
end

Voltlab.StartMinigame = function(time, callback)
    ...
    CreateThread(function()
        ...
        SendNUIMessage({
            openMinigameGif = true,
            minigameGif = "voltage_intro.gif",
        })
    ...
    CreateThread(function()
        ...
        while isHackActive do
            ...
            elseif IsControlJustPressed(0, 194) then -- backspace
                PlayVoltaFailureMinigame(callback)
                        ...
                        if confirmedNodes[tostring(activeNumberNode)] == nil or confirmedNodes[tostring(activeNumberNode)] == false then
                            ...
                            if getTableSize(confirmedNodes) >= 3 then
                                if totalVoltage == targetVoltage then
                                    ...
                                    SendNUIMessage({
                                        openMinigameGif = true,
                                        minigameGif = "voltage_success.gif",
                                    })
                                    ...
                                    PlayVoltaCompleteMinigame(callback)
                                else
                                    Wait(1000)
                                    PlayVoltaFailureMinigame(callback)
                                end
...
PlayVoltaCompleteMinigame = function(callback)
    resultFlashCount = 0
...
PlayVoltaFailureMinigame = function(callback)
    isSuccess = false
    isHackActive = false

    SendNUIMessage({
        openMinigameGif = true,
        minigameGif = "voltage_fail.gif",
    })
