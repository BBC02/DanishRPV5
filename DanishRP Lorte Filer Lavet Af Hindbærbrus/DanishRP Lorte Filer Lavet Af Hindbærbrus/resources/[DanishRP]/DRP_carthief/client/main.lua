...
function SpawnCar()
	ESX.TriggerServerCallback('troll_carthief:isActive', function(cooldown)
...
function FinishDelivery()
	...
	if GetPedInVehicleSeat(GetVehiclePedIsIn(PlayerPedId()), -1) == PlayerPedId() and GetEntitySpeed(vehicle) < 3 and GetDistanceBetweenCoords(coords, v.posx, v.posy, v.posz, true) < 10 and GetVehicleNumberPlateText(vehicle) == "        " and not isDeliving then
		...
		--Pay the poor fella
		TriggerServerEvent('troll_carthief:pay', securityToken, randomdelivery)
...
function AbortDelivery()
	...
	-- Reset cooldown
	TriggerServerEvent('troll_carthief:setcooldownabort', securityToken)
	cooldowntime = 0
...
AddEventHandler('troll_carthief:hasEnteredMarker', function(zone)
...
AddEventHandler('troll_carthief:hasExitedMarker', function(zone)
	CurrentAction = nil
...
Citizen.CreateThread(function()
    while true do
		...
		if isInMarker and not HasAlreadyEnteredMarker then
			...
			TriggerEvent('troll_carthief:hasEnteredMarker', currentZone)
		end
		if not isInMarker and HasAlreadyEnteredMarker then
			HasAlreadyEnteredMarker = false
			TriggerEvent('troll_carthief:hasExitedMarker', LastZone)
		end
	end
end)
...
Citizen.CreateThread(function()
	...
	local lmodel = (true) and GetHashKey('s_m_y_dealer_01') or GetHashKey(GetHashKey('s_m_y_dealer_01'))
	RequestModel(lmodel)
	while not HasModelLoaded(lmodel) do
		Wait(10)
	end
	lPed = CreatePed(4, lmodel, 758.2795, -3195.3259, 6.07, false, false)
	...
	TaskStartScenarioInPlace(lPed, "", 0, 0)
	SetModelAsNoLongerNeeded(lmodel)
end)

exports.qtarget:AddTargetModel({'s_m_y_dealer_01'}, {
	options = {
		{
			event = "startMission",
			icon = "fas fa-user-hard-hat",
			label = "Tilgå Jobcenter",
			num = 1
		},
        {
			event = "startMission",
			icon = "fas fa-user-hard-hat",
			label = "Tilgå Job muligheder",
			num = 2
		},
	},
	distance = 2.0
