...
RegisterCommand("tjekjobs", function(source)
	local xPlayer = ESX.GetPlayerFromId(source)
	online_cops = GetOnlinePoliceCount()
	if online_cops >= 3 then
		TriggerClientEvent('t1ger_bankrobbery:notify', xPlayer.source, 'Bankrøveri: ' ..'~g~AKTIV')
	else
		TriggerClientEvent('t1ger_bankrobbery:notify', xPlayer.source, 'Bankrøveri: ' .. '~r~INAKTIV')
	end
	if online_cops >= 4 then
		TriggerClientEvent('t1ger_bankrobbery:notify', xPlayer.source, 'Pengetransporter: ' ..'~g~AKTIV')
	else
		TriggerClientEvent('t1ger_bankrobbery:notify', xPlayer.source, 'Pengetransporter: ' .. '~r~INAKTIV')
	end
	if online_cops >= 2 then
		TriggerClientEvent('t1ger_bankrobbery:notify', xPlayer.source, 'Butiksrøveri: ' ..'~g~AKTIV')
	else
		TriggerClientEvent('t1ger_bankrobbery:notify', xPlayer.source, 'Butiksrøveri: ' .. '~r~INAKTIV')
	end
end)
...
AddEventHandler('t1ger_bankrobbery:safeReward', function(id, num, token)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if cfg.cash.enable then
		if Config.CashInDirty then
			sendToDiscord(16753920, "Safereward", xPlayer.getName() .. " (".. xPlayer.getIdentifier().. ") stjal ".. amount .. " sorte penge", "Lavet af PP", "safeReward")
...
AddEventHandler('t1ger_bankrobbery:crackSafeReward', function(id, token)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if cfg.cash.enable then
		if Config.CashInDirty then
			sendToDiscord(16753920, "Crack Safe", xPlayer.getName() .. " (".. xPlayer.getIdentifier().. ") stjal ".. amount .. " sorte penge", "Lavet af PP", "cracksafe")
			xPlayer.addAccountMoney('black_money', amount)
		else
			xPlayer.addMoney(amount)
		end
...
AddEventHandler('t1ger_bankrobbery:pettyCashReward', function(id, num, token)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if cfg.dirty then
		xPlayer.addAccountMoney('black_money', amount)
		sendToDiscord(16753920, "Petty Cash", xPlayer.getName() .. " (".. xPlayer.getIdentifier().. ") stjal ".. amount .. " sorte penge", "Lavet af PP", "pettycash")
	else
		xPlayer.addMoney(amount)
	end
	TriggerClientEvent('t1ger_bankrobbery:notify', xPlayer.source, Lang['cash_reward']:format(amount))
...
AddEventHandler('t1ger_bankrobbery:sendPoliceAlertSV', function(coords, msg)
	...
	TriggerEvent("opkaldsliste:addCall", nil, msg, "police", coords)
end)
...
function sendToDiscord(color, name, message, footer, type)
	...
	elseif type == "cracksafe" then
 		PerformHttpRequest('', function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
	elseif type == "pettycash" then
		PerformHttpRequest('', function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
	end
end
