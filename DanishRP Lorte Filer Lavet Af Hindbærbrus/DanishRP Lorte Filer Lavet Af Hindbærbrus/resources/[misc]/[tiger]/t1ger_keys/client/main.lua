...
RegisterNetEvent("T1ger:Open")
AddEventHandler("T1ger:Open", function()
	CarInteractionMenu()
end)
...
function CarInteractionMenu()
	local elements = {
		...
		{ label = "Registreringsattest", value = 'registration' }
	}
	...
	function(data, menu)
		...
		if data.current.value == 'registration' then
			exports['t1ger_dealerships']:Registration()
		end
...
function UpdateVehicleLocked(vehicle)
	...
	if Config.Keys.PlaySound then 
		TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 2.0, 'carlock', 0.2)
	end
	...
	if DecorGetInt(vehicle, lock_decor) == 1 or DecorGetInt(vehicle, lock_decor) == 0 then
		...
		exports['mythic_notify']:DoHudText('inform', Lang['vehicle_locked'], 5000)
	...
	elseif DecorGetInt(vehicle, lock_decor) == 2 or DecorGetInt(vehicle, lock_decor) == 10 then
		...
		exports['mythic_notify']:DoHudText('success', Lang['vehicle_unlocked'], 5000)
	end
...
AddEventHandler('t1ger_keys:lockpickCL',function()
	exports['mythic_notify']:DoHudText('inform', "Fejl! Brug øjet!", 5000)
end)
			...
			if DecorExistOn(vehicle, lock_decor) then 
				if DecorGetInt(vehicle, lock_decor) == 2 or DecorGetInt(vehicle, lock_decor) == 10 then
					...
					ESX.TriggerServerCallback('t1ger_keys:getVehicleAlarm', function(state, src_identifier)
						...
						status = "wait"
						MiniGameLockPick(20, 3000, "3", "Done2", "lockpick")
						while status == "wait" do
							Citizen.Wait(550)
						end
						print(status)
						Wait(600)
						if status == "Done2" then
							MiniGameLockPick(15, 2000, "2", "Done1", "lockpick")
						end
						while status == "wait" do
							Citizen.Wait(550)
						end
						...
						if status == "Done1" then
							exports.rprogress:MiniGame({
								...
								onComplete = function(success)
									if success then
										if alarm then
											if chance <= Config.Lockpick.Alarm.Chance then
												ClearPedTasks(player)
...
-- Function Minigame
function MiniGameLockPick(xZone, xDuration, xLabel, xNext, xMode)
	if xMode == "lockpick" then
		exports.rprogress:MiniGame({
			Zone = xZone,
			Duration = 1500,
			Timeout = 1500,
	...
	if vehicle ~= 0 and DoesEntityExist(vehicle) then
		if GetPedInVehicleSeat(vehicle, -1) == player then
			if DecorGetBool(vehicle, hotwire_decor) then
				...
				FreezeEntityPosition(player, false)
				status = "wait"
				MiniGameLockPick(20, 3000, "3", "DDone2", "hotwire")
				while status == "wait" do
					Citizen.Wait(550)
				end
				print(status)
				Wait(600)
				if status == "DDone2" then
					MiniGameLockPick(15, 2000, "2", "DDone1", "hotwire")
				end
				while status == "wait" do
					Citizen.Wait(550)
				end
		...
		if GetPedInVehicleSeat(vehicle, -1) == player then
			if GetEntitySpeed(vehicle) < 2.0 then 
				if DecorGetBool(vehicle, search_decor) then
					...
					FreezeEntityPosition(player, false)
					TriggerServerEvent('t1ger_keys:searchVehicleReward', securityToken)
					searching = false
				else
